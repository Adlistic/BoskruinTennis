...
public class SaveEvents : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        catch (UnauthorizedAccessException)
        {
            ...
            context.Response.Write("{\"success\":false,\"message\":\"Permission denied. Check file permissions on the server.\"}");
        }
        catch (Exception ex)
        {
            context.Response.StatusCode = 500;
            context.Response.Write("{\"success\":false,\"message\":\"Error saving events: " + ex.Message.Replace("\"", "'") + "\"}");
        }
    }
